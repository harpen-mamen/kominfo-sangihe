    ...
    <div class="space-y-6">
        <div class="grid gap-4 md:grid-cols-2">
            ...
            <label class="space-y-2">
                ...
                <select wire:model.live="indikatorId" class="w-full rounded-lg border-gray-300 bg-white text-sm shadow-sm dark:border-gray-700 dark:bg-gray-900">
                    @foreach ($this->allIndikatorOptions as $id => $label)
                        <option value="{{ $id }}">{{ $label }}</option>
                    @endforeach
                </select>
            </label>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    @foreach ($this->desaRows as $desa)
                        <tr>
                            ...
                            <td class="px-4 py-3">
                                <input type="number" step="0.01" wire:model.defer="nilai.{{ $desa->id }}.{{ $indikatorId }}" class="w-44 rounded-lg border-gray-300 bg-white text-sm shadow-sm dark:border-gray-700 dark:bg-gray-900">
                            </td>
                        </tr>
                    @endforeach
                </tbody>
